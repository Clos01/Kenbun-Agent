...
3. Optionally upload for a shareable link using `scripts/upload.py` via terminal.
...
- Filled: `backgroundColor: "#a5d8ff", fillStyle: "solid"`
...
> [!WARNING]
> Do NOT use `"label": { "text": "..." }` on shapes. This is NOT a valid Excalidraw property and will be silently ignored, producing blank shapes. You MUST use the container binding approach below.
...
### Labeled Arrow
Use same container binding approach:
```json
[
...
- Emit progressively: background zones &rarr; shape &rarr; its bound text &rarr; its arrows &rarr; next shape.
- **BAD:** all rectangles, then all texts, then all arrows.
- **GOOD:** `bg_zone` &rarr; `shape1` &rarr; `text_for_shape1` &rarr; `arrow1` &rarr; `arrow_label_text` &rarr; `shape2` &rarr; `text_for_shape2` &rarr; ...
