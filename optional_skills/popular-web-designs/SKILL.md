...
kenbun:
  mode: document
  fidelity: high
  tech_stack: [html, css, design-system]
...
Each template includes a **Kenbun Implementation Notes** block at the top with:
...
<head>
  ...
  <!-- Paste the Google Fonts <link> from the template's Kenbun notes -->
...
Write the file with `write_file`, serve with the generative-widgets workflow (cloudflared tunnel), and verify the result with browser tools to confirm visual accuracy.
...
When a template's CDN font matches the original (Inter, IBM Plex, Rubik, Geist), no substitution loss occurs. When a substitute is used, follow the template's weight, size, and letter-spacing values closely — those carry more visual identity than the specific font face.
...
- **opencode.ai.md** (OpenCode): Developer-centric dark theme, full monospace
