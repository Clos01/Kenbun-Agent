...
kenbun:
  mode: document
  fidelity: high
  tech_stack: [bash, python, curl, pyfiglet, cowsay, boxes, toilet]
...
> [!NOTE]
> toilet outputs ANSI escape codes for colors — works in terminals but may not render in all contexts (e.g., plain text files, some chat platforms).
...
## Decision Flow
1. **Text as a banner** &rarr; `pyfiglet` if installed, otherwise `asciified` API via `curl`
2. **Wrap a message in fun character art** &rarr; `cowsay`
3. **Add decorative border/frame** &rarr; `boxes` (can combine with `pyfiglet`/`asciified`)
4. **Art of a specific thing** (cat, rocket, dragon) &rarr; `ascii.co.uk` via `curl` + parsing
5. **Convert an image to ASCII** &rarr; `ascii-image-converter` or `jp2a`
6. **QR code** &rarr; `qrenco.de` via `curl`
7. **Weather/moon art** &rarr; `wttr.in` via `curl`
8. **Something custom/creative** &rarr; LLM generation with Unicode palette
9. **Any tool not installed** &rarr; install it, or fall back to next option
