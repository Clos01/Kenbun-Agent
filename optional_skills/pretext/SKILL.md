...
Because it is fast and geometric, it is a creative primitive: you can reflow paragraphs around a moving sprite at 60fps, build games whose level geometry is made of real words, drive ASCII logos through prose, shatter text into particles with exact per-grapheme starting positions, or pack shrink-wrapped multiline UI without any `getBoundingClientRect` thrash.

This skill exists so Kenbun can make cool demos with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for reference.

---

## When to Use
Use when the user asks for:
- A "pretext demo", "cool pretext thing", or "text-as-X" concept.
...
- Image &rarr; text (use `ascii-art` / `ascii-video` skills).
...
- **Beyond "Hello World":** The `hello-orb-flow.html` template is a starting point. Every delivered demo must add intentional color, motion, composition, and one visual detail the user didn't ask for but will appreciate.
- **Atmosphere:** Dark backgrounds, warm cores, considered palette. Classic amber-on-black (CRT / terminal) works, but so do cold-white-on-charcoal (editorial) and desaturated pastels (risograph). Pick one and commit.
- **Proportional Fonts:** Pretext's whole vibe is "not monospaced" — lean into it. Use Iowan Old Style, Inter, JetBrains Mono, Helvetica Neue, or a variable font. Never default sans.
- **Real Corpus:** Use real source/text, not lorem ipsum. Short manifestos, poetry, real source code, a found text, the library's own README — never lorem ipsum.
...
```
*Pin the version. `@0.0.6` is default — check npm for the latest if demo behavior is off.*

---

## The Two Use Cases
Almost everything reduces to one of these two shapes.
...
*Keep font and letterSpacing exactly in sync with your CSS. The canvas ctx.font format (e.g. "16px Inter") must match the rendered CSS, or measurements drift.*
...
let y = 0;
while (true) {
  const lineWidth = widthAtY(y);  // how wide is the corridor at this y?
...
```
*This is the most important pattern in the library. It's what unlocks "text flowing around a dragged sprite" — the demo that went viral on X.*

### Helpers Worth Knowing
- `measureLineStats(prepared, maxWidth)` &rarr; `{ lineCount, maxLineWidth }` — the widest line, i.e. multiline shrink-wrap width.
...
| **Shatter / particles** | `walkLineRanges` &rarr; per-grapheme (x,y) &rarr; physics | Sentence that explodes into letters on click |
...
3. Use `write_file` to a new `.html` in `/tmp/` or the user's workspace.
4. Swap the corpus for something intentional to the brief. Real prose, 10-100 sentences, no lorem.
5. Tune the aesthetic — font, palette, composition, interaction.
6. Verify locally:
   ```bash
   cd <dir-with-html> && python3 -m http.server 8765
   # then open http://localhost:8765/<file>.html
   ```
7. Check the console — pretext will throw if `prepareWithSegments` is called with a bad font string; `Intl.Segmenter` is available in every modern browser.
8. Show the user the file path, not just the code — they want to open it.
...
## Common Pitfalls
- **Drifting CSS/canvas font strings:** `ctx.font = "16px Inter"` measured, but CSS says `font-family: Inter, sans-serif; font-size: 16px;`. If Inter 404s, CSS falls back to sans-serif and measurements drift by 5-20%. Always preload the font or use a web-safe family.
- **Re-preparing inside the animation loop:** Only `layout*` is cheap. Re-calling prepare every frame will tank performance. Keep the prepared handle in module scope.
...
Clone these for inspiration / patterns (linked from `pretext.cool`):
