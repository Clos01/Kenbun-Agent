...
kenbun:
  mode: document
  fidelity: high
  tech_stack: [python, numpy, scipy, pillow, ffmpeg, opencv]
...
- **Creative Concept First:** Before writing a single line of code, articulate the creative concept. What is the mood? What visual story does this tell? What makes THIS project different from every other ASCII video? The user's prompt is a starting point — interpret it with creative ambition, not literal transcription.
- **First-Render Excellence:** First-render excellence is non-negotiable. The output must be visually striking without requiring revision rounds. If something looks generic, flat, or like "AI-generated ASCII art," it is wrong — rethink the creative concept before shipping.
- **Go Beyond Reference Vocabulary:** The effect catalogs, shader presets, and palette libraries in the references are a starting vocabulary. For every project, combine, modify, and invent new patterns. The catalog is a palette of paints — you write the painting.
...
| **Hybrid** | Video + audio | ASCII video with audio-reactive overlays | Both input references |
...
- **Signal:** SciPy (FFT, peak detection for audio modes)
...
`INPUT` &rarr; `ANALYZE` &rarr; `SCENE_FN` &rarr; `TONEMAP` &rarr; `SHADE` &rarr; `ENCODE`
...
- **Mode:** Select one of the 6 modes.
...
- Scene table + dispatcher — time &rarr; scene function + config (`references/scenes.md`)
...
- List its core visual/structural elements (erosion &rarr; gradual reveal; mitosis &rarr; splitting duplication; weaving &rarr; interlocking patterns).
