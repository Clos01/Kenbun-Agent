...
kenbun:
  mode: document
  fidelity: high
  tech_stack: [python, manim, latex, ffmpeg]
...
- **Narrative Arc First:** Before writing a single line of code, articulate the narrative arc. What misconception does this correct? What is the "aha moment"? What visual story takes the viewer from confusion to understanding? The user's prompt is a starting point — interpret it with pedagogical ambition.
...
- **Opacity Layering:** Opacity layering directs attention. Never show everything at full brightness. Primary elements at 1.0, contextual elements at 0.4, structural elements (axes, grids) at 0.15. The brain processes visual salience in layers.
...
`PLAN` &rarr; `CODE` &rarr; `RENDER` &rarr; `STITCH` &rarr; `AUDIO (optional)` &rarr; `REVIEW`
...
### Performance Targets
- **`-ql` (draft):** 854x480 @ 15fps &rarr; 5-15s/scene
- **`-qm` (medium):** 1280x720 @ 30fps &rarr; 15-60s/scene
- **`-qh` (production):** 1920x1080 @ 60fps &rarr; 30-120s/scene
...
| `references/paper-explainer.md` | Turning research paper text into mathematical animation — layouts, timings, graphs |
...
### 1. SCAMPER Transformation
Take a standard mathematical/technical visualization and transform it:
- **Substitute:** Replace the standard visual metaphor (e.g. number line &rarr; winding path, matrix &rarr; city grid).
- **Combine:** Merge two explanation approaches (e.g. algebraic + geometric simultaneously).
- **Reverse:** Derive backward — start from the result and deconstruct to axioms.
- **Modify:** Exaggerate a parameter to show why it matters (e.g. 10x the learning rate, 1000x the sample size).
...
- Reverse it (e.g. right-to-left derivation, 3D embedding of a 2D concept, continuous morphing instead of steps, zero notation).
