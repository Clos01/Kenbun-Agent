...
*Note: This skill is separate from the Kenbun Email gateway adapter. The gateway adapter lets people email the agent and uses Kenbun's built-in IMAP/SMTP adapter; this skill lets the agent operate a mailbox from terminal tools and requires the external himalaya CLI.*
...
# (inbox/sent/drafts/trash). Gmail is the common case.
...
### Reply to an Email
To reply non-interactively from Kenbun, read the original message, compose a reply, and pipe it:
```bash
...
### Write a New Email
Non-interactive (use this from Kenbun) — pipe the message via stdin:
```bash
cat << 'EOF' | himalaya template send
From: you@example.com
