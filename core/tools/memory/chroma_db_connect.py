...
# Note: chromadb is imported lazily inside functions to prevent slow module load times
...
def query_embeddings(query_text: str, n_results: int = 5, category: str = "code", filter_project: str = None, where: dict = None):
    ...
    try:
        ...
        if where:
            def _sanitize_where(where_val):
                if isinstance(where_val, dict):
                    clean_dict = {}
                    for k, v in where_val.items():
                        if str(k).startswith("$"):
                            logger.warning(f"⚠️ [CHROMA] Blocked nested NoSQL injection attempt: {k}")
                            continue
                        clean_dict[k] = _sanitize_where(v)
                    return clean_dict
                elif isinstance(where_val, list):
                    return [_sanitize_where(item) for item in where_val]
                return where_val

            for k, v in where.items():
                if str(k).startswith("$"):
                    logger.warning(f"⚠️ [CHROMA] Blocked NoSQL injection attempt: {k}")
                    continue
                filters.append({k: _sanitize_where(v)})
