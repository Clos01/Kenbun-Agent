...
def build_system_prompt(tier: str, llm_model: str) -> str:
    ...
    if tier == "nano":
        ...
        return (
            ...
            "2. Execute `kenbun orchestrate` in an execute block to dynamically route the task to the Cognitive Assembly.\n"
            "3. Execute `kenbun consult_supervisor` in an execute block to enforce System 2 safety guardrails.\n"
            "4. Always use `/recall <query>` to search Hivemind ChromaDB for previous context.\n"
            "You are the Terminal Gateway. Rely on the execute blocks to do the heavy lifting!\n"
        )
