...
Typical pipeline (run from the machine where Antigravity is installed;
substitute your own remote host and container name):

    python3 core/tools/sensory/antigravity_importer.py extract \
      | ssh <remote-host> docker exec -i <kenbun-container> \
          python3 /app/core/tools/sensory/antigravity_importer.py apply

Running everything on one machine works too:

    python3 core/tools/sensory/antigravity_importer.py extract \
      | python3 core/tools/sensory/antigravity_importer.py apply
