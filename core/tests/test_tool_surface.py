...
def test_scan_repo_returns_repo_map():
    ...
    cli_path = str(Path(__file__).parent.parent / "tools" / "cli")
    out = _ok(srv.scan_repo(cli_path))
...
def test_run_code_safely_works_or_skips_cleanly():
    ...
    if (
        "docker is not running" in out.lower()
        or "e2b api key is missing" in out.lower()
        or "sandbox failed" in out.lower()
        or "docker daemon" in out.lower()
    ):
        pytest.skip("docker daemon down or E2B API Key missing — external dependency, not tool rot")
    _ok(out)
