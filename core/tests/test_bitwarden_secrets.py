...
from tools.utils.secrets_bitwarden import (
    get_target_triple,
    get_bws_bin_name,
    load_hermes_config_raw,
    save_hermes_config_raw,
...
def test_load_save_config():
    with tempfile.TemporaryDirectory() as tmpdir:
        # Mock get_hermes_dir to point to tmpdir
        with patch("tools.utils.secrets_bitwarden.get_hermes_dir", return_value=tmpdir):
            config = {"secrets": {"bitwarden": {"enabled": True, "project_id": "test-id"}}}
            save_hermes_config_raw(config)
            
            loaded = load_hermes_config_raw()
