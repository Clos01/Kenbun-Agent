...
const WORKER_DETAILS: Record<string, WorkerDetails> = {
  "LM Studio": {
    ...
    activeModel: "Qwen2.5-Coder-7B-Instruct / Kenbun-3-Llama3.1-8B (Q4_K_M)",
    responsibilities: [
    ...
    <div className="min-h-screen bg-[var(--background)] flex selection:bg-[var(--gold)] selection:text-white max-w-[100vw] overflow-x-hidden">
      ...
      <main className="flex-1 p-0 relative flex flex-col transition-all duration-700 pb-20 lg:pb-0 min-w-0 overflow-x-hidden">
        ...
        >
          ...
          {budget && (() => {
            const isExternalSource = budget.source === 'kenbun_router';
            const hasRealSpend = budget.daily_usage > 0 || budget.lifetime_spend > 0;
            // Only show actual $ values if there's real tracked spend, otherwise label as External
            const todayDisplay = (!isExternalSource || hasRealSpend) ? `$${budget.daily_usage.toFixed(2)}` : 'External';
            const lifetimeDisplay = (!isExternalSource || hasRealSpend) ? `$${budget.lifetime_spend.toFixed(2)}` : 'External';
            const remainingDisplay = (!isExternalSource || hasRealSpend) ? `$${budget.remaining.toFixed(2)}` : `$${budget.daily_limit.toFixed(2)}`;
            const burnDisplay = hasRealSpend ? `${((budget.daily_usage / budget.daily_limit) * 100).toFixed(1)}%` : '—';
            return (
