...
| Location | Issue |
|---|---|
| `update_claude_config.py` | **Plaintext `GEMINI_API_KEY` committed to repo — rotate this key immediately.** Also hardcodes Tailscale host `remote-host.example`, IP `127.0.0.1`, LM Studio port `2065`, and `~/...` paths |
| `claude_desktop_config.json`, `test_mcp.py`, `test_mcp_tools.py` | Hardcoded `~/...` paths and Tailscale IPs |
| `core/tests/test_speculative_decoding.py:22` | Hardcoded fallback IP `127.0.0.1` |
| `core/scratch/test_models_direct.py` | Hardcoded Tailscale URL |
| `core/NEURAL_HIERARCHY.md` | Docs reference "Remote PC (127.0.0.1)" |
