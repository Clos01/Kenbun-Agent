...
post_installation_diagnostics() {
    ...
    echo -e "    7. ${GREEN}💡 STANDALONE CLI RE-CREATION (To rebuild/refresh or resolve conflicts):${NC}"
    echo -e "       If you encounter pre-existing container name conflicts or want to force a clean build, run:"
    echo -e "         ${CYAN}docker rm -f portable_dashboard portable_fastmcp portable_chroma portable_ollama portable_ollama_init portable_dozzle 2>/dev/null${NC}"
    echo -e "         ${CYAN}docker compose up -d --build --force-recreate${NC}"
    echo ""
    ...
    echo -e "    2. ${GREEN}🌸 AUTOMATED SEQUENTIAL ONBOARDING:${NC} Because this is a fresh setup, the CLI will"
    echo -e "       automatically guide you through a step-by-step sequential sequence (Steps 1 to 9):"
