...
post_installation_diagnostics() {
    ...
    echo -e "    2. Create the administrator account."
    echo -e "    3. Go to ${BOLD}Stacks${NC} -> ${BOLD}Add stack${NC}."
    echo -e "    4. Copy the content of '/opt/kenbun-agent/docker-compose.yml' into the Web Editor."
    echo -e "    5. Specify the environment variables by loading the contents of '/opt/kenbun-agent/.env'."
    echo -e "    6. Click ${BOLD}Deploy the stack${NC}."
    echo ""
    echo -e "  ${BOLD}Option B: Deploy via Interactive CLI Wizard${NC}"
    echo -e "    If you prefer terminal-only execution, simply run the setup wizard command to configure and start your swarm:"
    echo -e "    ${CYAN}kenbun${NC} (or ${CYAN}sudo bash /opt/kenbun-agent/install.sh${NC})"
    echo ""
